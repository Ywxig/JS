...
Все три метода перебирают массив, но делают разные вещи:



map() — преобразует каждый элемент и возвращает новый массив той же длины.
```js
jstransactions.map(t => t.transaction_amount);
// => [250, 1200, 89.99, 350, ...]  (10 элементов → 10 элементов)
```

filter() — отбирает элементы по условию и возвращает новый массив меньшей или равной длины.
```js
jstransactions.filter(t => t.transaction_type === "debit");
// => [{ T001 }, { T003 }, ...]  (10 элементов → 7 элементов)
```

reduce() — сворачивает весь массив в одно значение (число, строку, объект — что угодно).
```js
jstransactions.reduce((sum, t) => sum + t.transaction_amount, 0);
// => 5075.49  (10 элементов → 1 число)
```
